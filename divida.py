...
def adicionar_divida():
    nome = input("Nome da dívida: ").strip()
    if not nome:
        print("O nome é obrigatório.")
        return

    while True:
        entrada = input("Valor (ex: 1500.50): ").strip().replace(",", ".")
        try:
            valor = float(entrada)
            break
        except:
    ...
    data_venc = solicitar_data("Data de vencimento")
    ...
    cursor.execute(
        ...
        (nome, valor, data_venc.isoformat() if data_venc else None, descricao, criado)
    )
    conexao.commit()
    conexao.close()
...
3) Listar pendentes
4) Listar pagas
5) Mostrar detalhes (ID)
6) Marcar como paga (ID)
7) Editar dívida (ID)
8) Excluir dívida (ID)
9) Buscar
10) Relatório
11) Exportar CSV
12) Vencimentos (7 dias)
0) Sair
==================================
""")
